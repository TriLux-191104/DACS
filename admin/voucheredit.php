...
if (!isset($_GET['id']) || !is_numeric($_GET['id'])) {
    header("Location: voucherlist.php");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['csrf_token']) && $_POST['csrf_token'] === $_SESSION['csrf_token']) {
    ...
    if (empty($errors)) {
        ...
        if (!$adminId) {
            ...
            $result = $voucher->updateVoucher($voucher_id, $data, $adminId);
            if ($result !== false) {
                header("Location: voucherlist.php?success=Cập nhật voucher thành công");
                exit;
            } else {
                $errors[] = "Cập nhật voucher thất bại: " . htmlspecialchars($voucher->db->error);
            }
        }
    }
}
        ...
        <div class="row g-3">
            <div class="col-md-6">
                ...
                <input type="text" name="code" class="form-control" 
                       value="<?php echo htmlspecialchars($voucher_data['code']); ?>" required maxlength="50">
            </div>
            <div class="col-md-6">
                <label class="form-label">Loại giảm giá</label>
                <select name="discount_type" class="form-select" required>
                    <option value="percentage" <?php echo $voucher_data['discount_type'] == 'percentage' ? 'selected' : ''; ?>>Phần trăm (%)</option>
                    <option value="fixed" <?php echo $voucher_data['discount_type'] == 'fixed' ? 'selected' : ''; ?>>Giá trị cố định (đ)</option>
                </select>
            </div>
            <div class="col-md-6">
                <label class="form-label">Giá trị giảm giá</label>
                <input type="number" name="discount_value" class="form-control" 
                       step="0.01" min="0.01" value="<?php echo $voucher_data['discount_value']; ?>" required>
            </div>
            <div class="col-md-6">
                <label class="form-label">Giá trị đơn hàng tối thiểu (đ)</label>
                <input type="number" name="min_order_amount" class="form-control" 
                       step="1000" min="0" value="<?php echo $voucher_data['min_order_amount']; ?>">
            </div>
            <div class="col-md-6">
                <label class="form-label">Ngày bắt đầu</label>
                <input type="datetime-local" name="start_date" class="form-control" 
                       value="<?php echo date('Y-m-d\TH:i', strtotime($voucher_data['start_date'])); ?>" required>
            </div>
            <div class="col-md-6">
                <label class="form-label">Ngày kết thúc</label>
                <input type="datetime-local" name="end_date" class="form-control" 
                       value="<?php echo date('Y-m-d\TH:i', strtotime($voucher_data['end_date'])); ?>" required>
            </div>
            <div class="col-md-6">
                <label class="form-label">Giới hạn sử dụng (0 = không giới hạn)</label>
                <input type="number" name="usage_limit" class="form-control" 
                       min="0" value="<?php echo $voucher_data['usage_limit']; ?>">
            </div>
            <div class="col-md-6">
                <label class="form-label">Trạng thái</label>
                <select name="status" class="form-select" required>
                    <option value="active" <?php echo $voucher_data['status'] == 'active' ? 'selected' : ''; ?>>Hoạt động</option>
                    <option value="inactive" <?php echo $voucher_data['status'] == 'inactive' ? 'selected' : ''; ?>>Không hoạt động</option>
                </select>
            </div>
            <div class="col-md-12">
                <label class="form-label">Mô tả</label>
                <textarea name="description" class="form-control" rows="3"><?php echo htmlspecialchars($voucher_data['description']); ?></textarea>
            </div>
            <div class="col-12">
